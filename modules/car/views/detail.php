...
class View extends \Car\Tools\View
{
    ...
    public function booking($index)
    {
        ...
        $content .= '<table class="border data fullwidth"><tbody>';
        $content .= '<tr><th>{LNG_Usage details}</th><td>'.nl2br($index->detail).'</td></tr>';
        ...
        if ($index->chauffeur > 0) {
            ...
            $chauffeur = array(-1 => '{LNG_Self drive}', 0 => '{LNG_Not specified (anyone)}');
        ...
        if (!empty($index->approved_date)) {
            $content .= '<tr><th>{LNG_Approver}</th><td>'.$index->approver_name.'</td></tr>';
            $content .= '<tr><th>{LNG_Approval date}</th><td>'.Date::format($index->approved_date).'</td></tr>';
        }
