...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        return static::createQuery()
            ->select('V.detail', 'V.id', 'V.vehicle_id', 'V.begin', 'V.end', 'V.status', 'V.reason', $sql, 'R.color')
        ...
        if ($request->initSession() && $request->isReferer()) {
            ...
            if ($action === 'cancel' && $login) {
                ...
                $search = static::createQuery()
                    ...
                    ->toArray()
                    ->first('V.*', $q1);
                if ($search && $login['id'] == $search['member_id'] && self::canCancle($search['today'], $search['status'])) {
                    // ยกเลิกการจองโดยผู้จอง
                    $search['status'] = self::$cfg->car_cancled_status;
                    $search['approver_name'] = $login['name'];
                    $search['approved_date'] = date('Y-m-d H:i:s');
                    // อัปเดต
                    $db->update($reservation_table, $search['id'], array(
                        'status' => $search['status'],
                        'approver' => $login['id'],
                        'approved_date' => $search['approved_date']
                    ));
                    // log
                    \Index\Log\Model::add(0, 'car', 'Status', Language::find('BOOKING_STATUS', '', $search['status']).' ID : '.$search['id'], $login['id']);
    ...
    public function bookDetail($id)
    {
        ...
        $select = array('V.*', 'R.number', 'U2.name chauffeur_name', 'U2.phone chauffeur_phone',
            'U.name contact', 'U.phone', 'R.color', 'R.seats', 'A.name approver_name');
        $n = 1;
