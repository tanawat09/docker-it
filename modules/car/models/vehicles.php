...
class Model extends \Kotchasan\Model
{
     ...
     */
    public static function toDataTable($params = array())
    {
        $where = array(
            array('R.published', 1)
        );
        ...
        return $query->where($where)
            ->select('R.id', Sql::CONCAT($concat, 'number', ' '), 'R.color', 'R.detail', 'R.seats');
    }
    ...
    {
        $result = array();
        foreach (static::toDataTable()->cacheOn()->execute() as $item) {
